...
import { eq, desc, gt, gte, and, count, inArray } from 'drizzle-orm'
import { cacheTag, cacheLife } from 'next/cache'
import { Loader2 } from 'lucide-react'
import type { LeaderboardEntry, Season } from '@/lib/types'
import LeaderboardClient from './leaderboard-client'

async function fetchCachedLeaderboardRows(seasonId: string): Promise<LeaderboardEntry[]> {
  'use cache'
  cacheTag(`leaderboard-${seasonId}`)
  cacheLife('minutes')

  const leaderboardRows = await db.select({
    userId: users.id,
    name: users.name,
